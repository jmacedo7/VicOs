  ...
  return (
    <Link
      ...
      className={[
        ...
        selected ? "bg-blue-50 text-blue-700 shadow-sm ring-1 ring-blue-100/80" : "text-slate-600 hover:bg-white/80 hover:text-slate-950",
      ].join(" ")}
    >
      <span className={[
        "grid h-8 w-8 shrink-0 place-items-center rounded-[10px] transition-all duration-200",
        selected ? "bg-white text-blue-600 shadow-sm" : "bg-slate-100/70 text-slate-500 group-hover:bg-white group-hover:text-slate-700",
      ].join(" ")}>
...
function MobileNavLink({ item, active }: { item: NavItem; active?: string }) {
  ...
  return (
    <Link href={href} className={selected ? "flex min-w-0 flex-1 flex-col items-center gap-1 text-blue-600" : "flex min-w-0 flex-1 flex-col items-center gap-1 text-slate-400"}>
      <span className={selected ? "grid h-8 w-10 place-items-center rounded-xl bg-blue-50" : "grid h-8 w-10 place-items-center rounded-xl"}>
    ...
    <>
      <aside className="sticky top-0 hidden h-screen w-[252px] shrink-0 border-r border-slate-200/70 bg-white/62 lg:block lg:backdrop-blur-xl">
        <div className="px-5 pb-6 pt-7">
          <Link href="/dashboard" className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] font-bold uppercase tracking-[.2em] text-slate-400">Business OS</div>
            </div>
          </Link>
        </div>
        ...
        <nav className="px-3">
          ...
          <div className="space-y-1">{links.map((item) => <NavLink key={item[2]} item={item} active={active} />)}</div>
          <div className="my-5 h-px bg-slate-200/80" />
          <p className="px-3 pb-2 text-[10px] font-bold uppercase tracking-[.18em] text-slate-400">Organização</p>
      ...
      <div className="fixed inset-x-0 top-0 z-40 flex h-[62px] items-center justify-between border-b border-slate-200/70 bg-white/72 px-4 backdrop-blur-xl lg:hidden">
        <Link href="/dashboard" className="flex items-center gap-2.5">
          <div className="vicos-orb h-9 w-9 rounded-xl"><span className="text-xs font-black text-blue-600">V</span></div>
          <span className="text-lg font-black tracking-tight"><span className="text-blue-600">Vic</span>Os</span>
        </Link>
        <Link href="/settings" aria-label="Configurações" className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200/70 bg-white/75 text-slate-500">
          <Icon name="settings" size={17} />
        </Link>
      </div>

      <nav className="fixed inset-x-3 bottom-3 z-40 flex h-[68px] items-center gap-1 rounded-[22px] border border-white/85 bg-white/78 px-2 shadow-[0_14px_38px_rgba(15,23,42,.13)] backdrop-blur-xl lg:hidden">
        {mobileLinks.map((item) => <MobileNavLink key={item[2]} item={item} active={active} />)}
      </nav>
    </>
  );
}
