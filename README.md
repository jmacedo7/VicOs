...
`NEXT_PUBLIC_SITE_URL=https://vicos.vercel.app`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `MICROSOFT_CLIENT_ID`, `MICROSOFT_CLIENT_SECRET` e `EMAIL_TOKEN_ENCRYPTION_KEY`.
...
3. No Supabase Auth, use `https://vicos.vercel.app` como Site URL e `https://vicos.vercel.app/auth/callback` na Redirect URL Allow List; mantenha o callback do Supabase (`https://muzopxphnxzxszkgpxoq.supabase.co/auth/v1/callback`) no provedor Google.
